...
locals {
  ...
  request_routing_rule1_name_app1 = "${azurerm_virtual_network.vnet.name}-rqrt-app1"
  ...
  request_routing_rule1_name_app2 = "${azurerm_virtual_network.vnet.name}-rqrt-app2"
...
resource "azurerm_application_gateway" "web_ag" {
  ...
  # Listerner: HTTP Port 80 AAP1
  http_listener {
    name                           = local.listener_name_app1
    frontend_ip_configuration_name = local.frontend_ip_configuration_name
    frontend_port_name             = local.frontend_port_name
    protocol                       = "Http"
    host_names                     = ["app1.digitaldense.com"]
  }

  # Listerner: HTTP Port 80 APP2
  http_listener {
    name                           = local.listener_name_app2
    frontend_ip_configuration_name = local.frontend_ip_configuration_name
    frontend_port_name             = local.frontend_port_name
    protocol                       = "Http"
    host_names                     = ["app2.digitaldense.com"]
  }
